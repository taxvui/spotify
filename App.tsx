...
const ScrollWrapper = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        <div className="flex-1 flex flex-col h-full relative bg-spotify-dark rounded-lg overflow-hidden my-2 mr-2 transition-colors duration-300">
            <TopBar opacity={opacity} />
            <div 
                ref={scrollRef}
                className="flex-1 overflow-y-auto bg-spotify-dark relative scroll-smooth transition-colors duration-300"
            >
                {/* Add a subtle gradient background at the top that persists */}
                <div className="absolute top-0 left-0 w-full h-80 bg-gradient-to-b from-[#202020] to-transparent -z-10" />
                {children}
            </div>
        </div>
    );
...
const AppContent = () => {
    ...
    return (
        <div className={`flex flex-col h-screen w-screen bg-black text-white font-sans ${isHighContrast ? 'high-contrast' : ''}`}>
            <div className="flex-1 flex overflow-hidden">
                ...
                <ScrollWrapper>
                    ...
                    {/* Fake Footer Links */}
                    <div className="p-8 pb-16 flex flex-col md:flex-row justify-between text-spotify-grey text-sm mt-8">
