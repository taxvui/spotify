...
const HomeIcon = ({ active }: { active: boolean }) => (
  <svg role="img" height="24" width="24" aria-hidden="true" viewBox="0 0 24 24" fill={active ? "white" : "currentColor"} className={active ? "" : "hover:text-white transition-colors"}>
...
const PlusIcon = () => (
    <svg role="img" height="16" width="16" aria-hidden="true" viewBox="0 0 16 16" fill="currentColor" className="hover:text-white transition-colors">
    ...
    <nav className="w-64 bg-black flex flex-col h-full gap-2 p-2 hidden md:flex">
      ...
      <div className="bg-spotify-dark rounded-lg flex-1 flex flex-col overflow-hidden">
        <div className="p-4 shadow-lg z-10">
          <div className="flex items-center justify-between mb-4 text-spotify-grey">
             <div className="flex items-center gap-2 hover:text-white font-bold cursor-pointer transition-colors">
                <LibraryIcon active={false} />
                Your Library
             </div>
             <div className="flex items-center gap-2">
                <button className="hover:bg-spotify-highlight p-1 rounded-full"><PlusIcon /></button>
                <button className="hover:bg-spotify-highlight p-1 rounded-full"><ArrowRightIcon /></button>
             </div>
          </div>
          <div className="flex gap-2 mb-2">
            <button className="bg-spotify-highlight hover:bg-spotify-hover text-white text-xs px-3 py-1 rounded-full transition-colors">Playlists</button>
            <button className="bg-spotify-highlight hover:bg-spotify-hover text-white text-xs px-3 py-1 rounded-full transition-colors">Artists</button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-2">
            <div className="flex flex-col">
                {playlists.map((playlist) => (
                    <div key={playlist.id} className="flex items-center gap-3 p-2 rounded-md hover:bg-spotify-highlight cursor-pointer group">
